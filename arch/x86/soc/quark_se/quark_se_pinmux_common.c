/* quark_se_pinmux_common.c - general pinmux operation */
...
int quark_se_pinmux_initialize_common(struct device *port, uint32_t *mux_config)
{
	...
	PIN_CONFIG(mux_config, 15, PINMUX_FUNC_B);
	PIN_CONFIG(mux_config, 16, PINMUX_FUNC_C);
	PIN_CONFIG(mux_config, 17, PINMUX_FUNC_C);
#if !defined CONFIG_UART_NS16550
	PIN_CONFIG(mux_config, 18, PINMUX_FUNC_B);
	PIN_CONFIG(mux_config, 19, PINMUX_FUNC_B);
#endif
#if defined CONFIG_UART_NS16550
